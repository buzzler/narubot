...
def initialize_whisper() -> WhisperModel:
    ...
    The model is initialized with a compute type of "float32".
    
    Returns:
        WhisperModel: An instance of the WhisperModel class configured as specified.
    """
    return WhisperModel(WHISPER, device=DEVIDE, compute_type="float32")
...
def initialize_tts():
    """
    Initializes a text-to-speech (TTS) engine to convert text into speech.

    This function uses the pyttsx3 library to set up the TTS engine and adjusts the speaking rate (rate) and volume (volume).

    Returns:
        tuple: A tuple containing the initialized TTS engine and the speaker ID.
    """
    from melo.api import TTS
    model = TTS(language='KR', device=DEVIDE)
    speaker_id = model.hps.data.spk2id['KR']
    return model, speaker_id

def initialize_tts_pyttsx3():
    """
...
def speech_to_text(file_path:str):
    ...
    This function takes a path to an audio file, reads it, and uses the configured Whisper model to transcribe the audio into text. The transcription is returned as a single string of text.

    Args:
        file_path (str): The path to the audio file that needs to be transcribed.
        
    Returns:
        str: A concatenated string of all segments' texts from the audio file, joined by spaces.
    """
    audio_file = open(file_path, "rb")
    segments, _ = stt.transcribe(audio_file, language="ko")
...
def text_to_speech(text):
    ...
    Returns:
        None
    """
    tts.tts_to_file(text, speaker_id, speed=1.3, quiet=True, play_audio=True)
